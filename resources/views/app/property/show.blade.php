                ...
                <div class="mb-6">
                    <div>
                        ...
                        @if ($property->owner())
                            @php
                                $user = $property->owner();
                                $owner = App\Models\Owner::where('user_id', $user->id)->where('property_id', $property->id)->first();
                            @endphp
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    <x-jet-label for="name" :value="__('Name')" />
                                    <x-jet-input id="name" class="block mt-1 w-full" type="text" value="{{ $user->name }}"
                                        disabled />
                                </div>
                                <div>
                                    <x-jet-label for="email" :value="__('Email')" />
                                    <x-jet-input id="email" class="block mt-1 w-full" type="email" value="{{ $user->email }}"
                                        disabled />
                                </div>
                                <div>
